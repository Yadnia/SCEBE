...
public class EditarGrupoCForm extends JDialog {
    ...
    private GrupoCultura grupoExistente;

    private boolean saved = false;

    public EditarGrupoCForm(Window owner, GrupoCultura grupoExistente) {
        super(owner, "Editar grupo deportivo", ModalityType.APPLICATION_MODAL);
        this.grupoExistente = grupoExistente;
        init();
        // Poner icono
    ...
    private void init() {
        ...
        JLabel title = new JLabel("Editar grupo deportivo");
        ...
        List<Estudiante> ests = EstudiantesController.getCulturaEstudiantes();
        ...
        // ❗ No limpiar selección al editar
        // cargarDatosExistentes() hará la selección correcta
    ...
    private void cargarDatosExistentes() {
        if (grupoExistente == null) return;

        tfNombre.setText(grupoExistente.getNombre());
        cbCategoria.setSelectedItem(grupoExistente.getCategoria().getNombre());

        DefaultTableModel model = (DefaultTableModel) tablaEstudiantes.getModel();
        ListSelectionModel selectionModel = tablaEstudiantes.getSelectionModel();
        selectionModel.clearSelection();

        for (int i = 0; i < model.getRowCount(); i++) {
            String carnet = (String) model.getValueAt(i, 2);
            boolean pertenece = grupoExistente.getGruposCulturaEstudiantes().stream()
                    .anyMatch(gde -> gde.getEstudiante().getCarnet().equals(carnet));
            if (pertenece) {
    ...
    private void guardarGrupo() {
        ...
        for (int r : sel) {
            ...
            if (asignacion != null) {
                if (asignacion.getGrupoCultura().getId() != grupoExistente.getId()) {
                    JOptionPane.showMessageDialog(this,
        ...
        grupoExistente.setNombre(tfNombre.getText().trim());
        grupoExistente.setCategoria(categoriaSel);
        CulturaController.updateGrupoCultura(grupoExistente);

        if (grupoExistente.getGruposCulturaEstudiantes() != null) {
            grupoExistente.getGruposCulturaEstudiantes().clear();
        }
        ...
        for (Estudiante estSel : estudiantesSeleccionados) {
            ...
            gde.setGrupoCultura(grupoExistente);
